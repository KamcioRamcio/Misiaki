...
static void handle_message(int tag, int from, const wire_msg_t *msg) {
  ...
  switch (tag) {
  case MSG_REQ: {
    log_debug("recv REQ od %d (ts=%d, m=%d, dock=%d)", from, msg->ts, msg->m,
              msg->dock);
...
static void try_enter_section(void) {
  ...
  if (!MY_REQ)
    return;
  if (!clock_w1_satisfied(MY_TS))
    return;
  if (!dock_w2_satisfied(&Q, MY_REQ))
    return;
  if (!mech_w3_satisfied(&Q, MY_REQ, M, K))
    return;
...
static void start_request(void) {
  ...
  state = STATE_TRYING;
  log_state("Rozpoczynam staranie o sekcję (m=%d, dock=%d, ts=%d)", MY_M,
            MY_DOCK, MY_TS);
...
int main(int argc, char **argv) {
  ...
  if (K <= 0 || M <= 0) {
    if (MY == 0)
      fprintf(stderr, "K i M muszą być dodatnie.\n");
    MPI_Finalize();
    return 1;
  }
  srand(seed);
