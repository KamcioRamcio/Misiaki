...
static void ensure_type(void) {
    if (WIRE_T != MPI_DATATYPE_NULL) return;
    ...
    for (int i = 0; i < 4; ++i) disps[i] -= base;
...
bool comm_try_recv(int *tag, int *from, wire_msg_t *msg) {
    ...
    MPI_Recv(msg, 1, WIRE_T, st.MPI_SOURCE, st.MPI_TAG,
             MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    if (tag)  *tag = st.MPI_TAG;
    if (from) *from = st.MPI_SOURCE;
    return true;
}
